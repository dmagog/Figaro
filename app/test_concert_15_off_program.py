#!/usr/bin/env python3
"""
Тест для проверки офф-программы перед концертом 15
"""
...
def test_concert_15_off_program():
    """Тест поиска офф-программы перед концертом 15"""
    print("🧪 Тестируем поиск офф-программы перед концертом 15...")
    
    session = next(get_session())
    
    try:
        # Получаем концерт 15
        concert_15 = session.exec(
            select(Concert).where(Concert.id == 15)
        ).first()
        
        if not concert_15:
            print("❌ Концерт 15 не найден в базе данных")
            return False
        
        # Извлекаем данные из Row объекта если нужно
        if hasattr(concert_15, '_mapping'):
            concert_data = concert_15._mapping['Concert']
        else:
            concert_data = concert_15
        
        print(f"📅 Концерт 15: {concert_data.name}")
        ...
        print(f"\n✅ Найдено мероприятий до концерта 15: {len(events)}")
        
        if events:
            print("\n📋 Мероприятия до концерта 15:")
            for event in events:
            ...
            if day_events:
                ...
                for event in day_events:
                    ...
                    print(f"     ⏱️ Длительность: {event_data.event_long}")
...
if __name__ == "__main__":
    print("🚀 Запуск теста офф-программы перед концертом 15")
    print("=" * 60)
    
    if test_concert_15_off_program():
        print("\n🎉 Тест прошел успешно!")
    else:
        print("\n⚠️ Тест не прошел") 