...
По этапам — [docs/06-mvp-roadmap.md](docs/06-mvp-roadmap.md). Версия **3.16.0** — **MVP завершён** (этапы 0–7) + офф-программа + **веб-слой** (светлая/тёмная тема с переключателем, фирменный шарик, пиктограммы; полный авторизационный контур; анкета, подбор под анкету, маршрутный лист карточками с номером концерта и содержательными подсказками переходов, админский пульт эмуляции, дашборды исследователя; server-rendered + HTMX). Дальше: телеграм-бот, live-API CRM.
