...
По этапам — [docs/06-mvp-roadmap.md](docs/06-mvp-roadmap.md). Версия **3.9.0** — **MVP завершён** (этапы 0–7: каркас, фестиваль+импорт, движок маршрутов, аккаунты/безопасность/анкета, подбор+лист+подсказки, наличие+эмуляция, уведомления, импорт остатков) + офф-программа (импорт и подсказки) + **веб-слой, первый срез** (упрощённый вход, подбор маршрутов, редактирование листа с подсказками — server-rendered + HTMX). Дальше: анкета и эмуляция в вебе, дашборды исследователя, бот, live-API CRM.
