...
По этапам — [docs/06-mvp-roadmap.md](docs/06-mvp-roadmap.md). Версия **3.17.0** — **MVP завершён** (этапы 0–7) + офф-программа + **веб-слой** (светлая/тёмная тема с переключателем, фирменный шрифт и шарик, Bootstrap-иконки; полный авторизационный контур; анкета, подбор под анкету, маршрутный лист карточками с номером концерта, содержательными подсказками переходов и дополнением по щелям, админский пульт эмуляции, дашборды исследователя с метриками доступности; server-rendered + HTMX). Дальше: телеграм-бот, live-API CRM.
