...
По этапам — [docs/06-mvp-roadmap.md](docs/06-mvp-roadmap.md). Версия **3.12.0** — **MVP завершён** (этапы 0–7) + офф-программа + **веб-слой** (регистрация+верификация, вход, анкета холодного старта, подбор под анкету, редактирование листа с подсказками, **админский пульт эмуляции** — FestivalClock + режим наличия; server-rendered + HTMX). Дальше: дашборды исследователя, телеграм-бот, live-API CRM.
